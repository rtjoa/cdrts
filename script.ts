...
// Discriminated union for the node type
type TextNode =
    | { type: 'Data'; value: string }
    | { type: 'Tombstone' };

interface Id { peer_id: number; local_id: number };
...
let el_input1: HTMLTextAreaElement;
let el_input2: HTMLTextAreaElement;
let el_tree1: HTMLDivElement;
let el_tree2: HTMLDivElement;
let el_lastEdits1: HTMLDivElement;
let el_lastEdits2: HTMLDivElement;
let el_output: HTMLDivElement;
let el_updateButton: HTMLButtonElement;

window.onload = () => {
    el_input1 = document.getElementById('input1') as HTMLTextAreaElement;
    el_input2 = document.getElementById('input2') as HTMLTextAreaElement;
    el_tree1 = document.getElementById('tree1') as HTMLDivElement;
    el_tree2 = document.getElementById('tree2') as HTMLDivElement;
    el_lastEdits1 = document.getElementById('last-edits-1') as HTMLDivElement;
    el_lastEdits2 = document.getElementById('last-edits-2') as HTMLDivElement;
    el_output = document.getElementById('output') as HTMLDivElement;
    el_updateButton = document.getElementById('updateButton') as HTMLButtonElement;
...
let state1: PeerState = init_state(1);
let state2: PeerState = init_state(2);
...
function createIdMap(tree: TextTree): Map<string, TextTree> {
    const map = new Map<string, TextTree>();

    function addToMap(node: TextTree) {
...
function merge(tree: TextTree, edits: Edit[]): TextTree {
...
function repr_tree(tree: TextTree, indent: number): string {
...
function tree_to_string(tree: TextTree): string {
...
type Edit =
    | { type: 'Insert'; parent: Id; node: TextTree }
    | { type: 'Delete'; index: Id }

function preorder_tree(tree: TextTree): TextTree[] {
    let result: TextTree[] = [];
    result.push(tree);
    for (const child of tree.children) {
...
function repr_edit(edit: Edit): string {
    if (edit.type === 'Insert') {
        let new_node = edit.node.node as { type: 'Data', value: string };
 ...
 */
function parse_edits(state: PeerState, text_with_edits: string, tree: TextTree): Edit[] | string {
    let edits: Edit[] = [];
    ...
    while (text_i < text_with_edits.length) {
        if (text_with_edits[text_i] === '-') {
            ...
            let node = nodes[node_i].node as { type: 'Data', value: string };
